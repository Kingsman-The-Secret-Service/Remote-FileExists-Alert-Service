...
class server(UiSample):
    ...
    def saveMail(self, mailValue):
        # self.constant.logComment('saveMail')
        smtpData = self.dbHandler.readSmtpData()
        # self.constant.logComment('smtpData = '+str(smtpData))
        newServerData = {
        ...
        # self.constant.logComment('smtpData = ' + str(smtpData))
        if not self.validateServerFormOnSubmit(newServerData):
            newServerData['smtp'] = smtpData['smtp']
            newServerData['smtpPort'] = smtpData['smtpPort']
